...
export async function publishSession(
  ...
  try {
    ...
    const dateStr = formatEventDate(session.eventDate)
    const eventTypeName = session.eventType?.name ?? ''
    const prTitle = `${dateStr} ${eventTypeName}`
    const prBody = `${dateStr}に実施した『${eventTypeName}』の報告記事を追加します

---
<sup>Generated by umineco-article-supporter</sup>`
